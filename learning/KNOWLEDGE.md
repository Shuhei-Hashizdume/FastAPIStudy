...
## Issue・ブランチ・Pull Requestの基本フロー

- 状態：基礎学習完了・経過観察
- Issueは問題、目的、完了条件を共有し、作業の方向と終了判断をそろえる
- Issueの完了条件を、調査・変更・確認の実行可能なタスクへ分解する
- ブランチはプロジェクトのコピーではなく、特定のコミットを指すGit上の目印である
- `git switch -c`で専用ブランチを作り、`git add`で次のコミットへ含める変更を選び、`git commit`で履歴へ保存する
- pushはローカルのコミットをGitHub上の作業ブランチへ共有する操作であり、`main`への反映ではない
- Pull Requestは作業ブランチの変更をレビューしてもらい、`main`への取り込みを提案する仕組みである
- PR本文には変更理由、変更内容、pytest・Ruff・mypyなどの確認結果を記載する
- `Closes #1`のように書くと、PRがマージされたときに対応するIssueを自動で閉じられる
- マージコミットは、作業ブランチの変更を`main`へ統合した履歴を表す
- マージ後は`git switch main`と`git pull --ff-only`でローカル`main`を更新し、マージ済みのローカル・リモート作業ブランチを削除する
- 今後の確認：別Issueでヒントなしに同じ流れを再現し、実際のレビュー指摘へ追加コミットで対応する

## Pull Requestのレビュー指摘への対応

- 状態：基礎学習完了・経過観察
- レビュー指摘を受けたら、すぐ修正せず、現在の問題とレビュアーが求める変更を自分の言葉で整理する
- 同じPRに関する修正は同じ作業ブランチへ追加コミットし、pushすると既存PRの差分が自動更新される
- 修正後は変更内容に応じて確認範囲を決め、文書だけの変更では移動元・移動先・重複・`git diff --check`を確認した
- Pythonコードを変更していないためpytestを実施しない判断をし、PRコメントへ未実施理由を明記した
- 返信には指摘の理解、修正内容、確認結果、未実施確認と理由を書く
- PR #3で修正、追加コミット、push、コメント、再レビュー相当の確認、マージ、ブランチ整理まで実施した
- 今後の確認：別のコード変更でMust Fixを受け、対象テストと回帰確認を含めて自力対応する

## 公式ドキュメントの調査

- 状態：基礎学習完了・経過観察
- 疑問をクラス名・引数名・確認したい動作まで具体化してから検索する
- 公式ドメインを確認したうえで、使用中のライブラリバージョンとドキュメントの対象バージョンを照合する
- `dev`は開発中、`latest`は現在の最新安定版を指すが将来変わり得る。固定バージョンURLがあれば再現性を保ちやすい
- 検索エンジンで`dev`版が上位に出る場合があるため、公式サイトのバージョン表示・選択メニュー・サイト内検索を使う
- 翻訳で全体像をつかみ、クラス名・引数名・型・デフォルト値・戻り値は原文とコード表記で照合する
- FastAPI公式から`HTTPBearer.auto_error`は`bool`・デフォルト`True`で、`False`なら認証情報なしの依存関係結果が`None`になることを確認した

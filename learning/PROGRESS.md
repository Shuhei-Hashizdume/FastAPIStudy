...
- 現在のプロジェクト：書籍管理API
- 今回の終了地点：EC2停止前の状態確認、安全な停止、再起動後の復旧、Docker VolumeのDBデータ永続化、パブリックIPv4の変化、IAM最小権限化を確認
- 現在の学習状態：学習用EC2の停止・再起動とデータ永続化の実測、IAM最小権限化は完了。HTTP通信をHTTPS化する学習から再開する
- 次回講義：HTTPとHTTPSの違いを整理し、学習用EC2に合うHTTPS化方針を選択する

## 今回完了した項目

- EC2停止前にコンテナなどの状態を確認し、DockerコンテナとEC2を安全に停止した
- EC2再起動後のDocker・Compose・API・PostgreSQLの復旧状態を実測した
- EC2停止前のDBデータが、再起動後もDocker Volumeに残っていることを確認した
...
- クラウドへの初回配置、停止・復旧、DB永続化、IAM最小権限化は完了。HTTPSとバックアップは未完了

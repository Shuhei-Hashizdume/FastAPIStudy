...
- CaddyをHTTPSの外部窓口として追加し、Compose内の`api:8000`へ転送した
- HTTP 80番からHTTPS 443番への308リダイレクトをMacから確認した
...
- クラウドへの初回配置、停止・復旧、DB永続化、IAM最小権限化、学習用HTTPS化は完了。ドメインと公的CAによる一般公開、バックアップ、監視は未完了
